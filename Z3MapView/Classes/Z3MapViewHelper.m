...
- (void)openMapWithCoordinate:(CLLocationCoordinate2D)coordiante
                    addressDictionary:(NSDictionary *)addressDictionary{
    NSArray *apps = [self appsForMapWithCoordiante:coordiante];
    NSArray *titles = [apps valueForKey:@"title"];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    for (NSString *title in titles) {
        UIAlertAction *action = [UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            if ([action.title isEqualToString:@"苹果地图"]) {
                [self openAppleMapWithCoordinate:coordiante addressDictionary:addressDictionary];
            }else {
                NSUInteger index =  [titles indexOfObject:action.title];
                NSDictionary *info = apps[index];
                NSString *url = info[@"url"];
...
- (void)openAppleMapWithCoordinate:(CLLocationCoordinate2D)coordinate
                  addressDictionary:(NSDictionary *)addressDictionary {
    MKMapItem *currentLoc = [MKMapItem mapItemForCurrentLocation];
    MKMapItem*toLocation = [[MKMapItem alloc] initWithPlacemark:[[MKPlacemark alloc] initWithCoordinate:coordinate addressDictionary:addressDictionary]];
    NSArray*items = @[currentLoc,toLocation];
...
-(NSArray*)appsForMapWithCoordiante:(CLLocationCoordinate2D)coordiante
{
    CLLocationCoordinate2D destination = coordiante;
    NSMutableArray*apps = [NSMutableArray array];
    NSMutableDictionary*iosMapDic = [NSMutableDictionary dictionary];
    iosMapDic[@"title"] =@"苹果地图";
    [apps addObject:iosMapDic];
